...
@else
    <form action="{{ url('/vendorSertif/' . $vendorSertifikasi->id_vendor_sertifikasi . '/delete_ajax') }}" method="POST" id="form-delete">
        @csrf
        @method('DELETE')
        <div id="modal-master" class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status) {
                                ...
                                dataVendorSertif.ajax.reload(); // Assuming you have a DataTable with this ID
                            } else {
                                $('.error-text').text('');
